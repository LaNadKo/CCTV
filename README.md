...
Комплексная система видеонаблюдения с поддержкой распознавания лиц, детекции движения, записи видео и многопользовательского управления. Включает серверную часть, веб-интерфейс, десктопный клиент и мобильный модуль, который пока сохранён в репозитории, но не является текущим приоритетом разработки.
...
| **Десктоп** | Electron 36, electron-builder, Inno Setup (установщик Windows) |
...
cd desktop
npm install
npm run build:win              # Frontend + Electron packaging (NSIS + portable)
# portable для тестов: desktop/release/win-unpacked/
# установщик: desktop/release/
```

> На Windows portable-сборка для тестирования берётся из `desktop/release/win-unpacked`. Если `electron-builder` упрётся в ограничение `winCodeSign`/symlink, эта папка всё равно обычно собирается корректно.
...
### Текущий фокус
- Основная итерация сейчас идёт по desktop/web-клиенту и Processor.
- Mobile-клиент оставлен на потом и может заметно отставать от desktop/web по возможностям и UX.
- Основная зона активной доработки: обнаружение, обработка камер и медиапайплайн через `Processor`.

### Уже работает
- Backend на `FastAPI` с авторизацией, ролями, группами камер, пользователями и процессорами
- Desktop-клиент на `Electron` и веб-клиент на `React/Vite/PWA`
- Processor c GUI, подключением по коду, heartbeat и обработкой камер
- Live, Recordings, Review, Persons, Reports, Users, Settings и Help в desktop/web-клиенте
- Архив записей с таймлайном по суткам, часовыми метками и запретом перехода в будущие даты
- Review с выбором существующей персоны через всплывающее окно, поиском по базе и созданием новой карточки из snapshot
- Persons с неточным поиском, редактированием карточки и live-сбором эмбеддингов в ручном режиме и через автосбор
- Settings с мгновенным применением интерфейсных настроек и перестановкой вкладок быстрого доступа
- Help, разбитая по вкладкам, с поиском по темам и краткими демонстрациями интерфейса
...
- Обнаружение и логика работы `Processor`: текущий приоритет следующей итерации
- Review/media-flow: snapshots и медиапроксирование уже переведены на `Processor-first`, но часть UX ещё шлифуется
- Mobile-клиент: сохранён в репозитории, но пока не синхронизируется с desktop/web на каждом шаге
...
Нативное приложение на React Native (Expo). Модуль сохранён в репозитории, но на текущем этапе не является приоритетным направлением и может отставать от desktop/web. Экраны:
