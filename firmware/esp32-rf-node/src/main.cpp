...
#ifndef NODE_LABEL
#define NODE_LABEL "cctv-rf-node"
#endif
...
constexpr size_t kPeerTextMax = 512;
constexpr size_t kMaxCsiBytes = 384;
constexpr size_t kCsiQueueSize = 36;
constexpr uint32_t kTdmSlotMs = 120;
constexpr uint8_t kBurstPerPeer = 3;
constexpr uint32_t kEspNowCsiInferWindowMs = 280;
constexpr uint8_t kEspNowCsiInferBudget = 4;
...
String hostname() {
  char buffer[40];
  snprintf(buffer, sizeof(buffer), "cctv-rf-node-%02u", config.node_id);
...
void loadConfig() {
  ...
  config.peers = prefs.isKey("peers") ? prefs.getString("peers", NODE_PEERS) : NODE_PEERS;
  prefs.end();
...
void saveConfigValue(const String &key, const String &value) {
  ...
  if (key == "node_id") {
    ...
    prefs.putString("peers", value.substring(0, kPeerTextMax));
  }
  prefs.end();
}
...
void parsePeers() {
  peer_count = 0;
  int start = 0;
  while (start >= 0 && start < config.peers.length() && peer_count < kMaxPeers) {
    const int end = config.peers.indexOf(';', start);
    const String item = (end >= 0 ? config.peers.substring(start, end) : config.peers.substring(start));
    start = end >= 0 ? end + 1 : -1;
    if (item.length() == 0) {
      continue;
    }

    const int first = item.indexOf('@');
    const int second = item.indexOf('@', first + 1);
    if (first <= 0) {
      continue;
    }

    PeerConfig peer;
    peer.node_id = static_cast<uint8_t>(item.substring(0, first).toInt());
    const String mac_text = second > first ? item.substring(first + 1, second) : item.substring(first + 1);
    if (peer.node_id == 0 || !parseMac(mac_text, peer.mac)) {
      continue;
    }
    if (second > first) {
      peer.has_ip = peer.ip.fromString(item.substring(second + 1));
    }
    peers[peer_count++] = peer;
  }
}

uint8_t peerIdByMac(const uint8_t mac[6]) {
  for (size_t i = 0; i < peer_count; ++i) {
    if (memcmp(peers[i].mac, mac, 6) == 0) {
      return peers[i].node_id;
    }
  }
  return 0;
}
...
String configJson(bool include_secret = false) {
  ...
  body.reserve(1024);
  body += "{";
  body += "\"firmware\":\"cctv-rf-csi-node\"";
  body += ",\"version\":\"0.2.0\"";
  ...
  for (size_t i = 0; i < peer_count; ++i) {
    ...
    body += "{";
    body += "\"node_id\":" + String(peers[i].node_id);
    body += ",\"mac\":" + q(macToString(peers[i].mac));
    body += ",\"ip\":" + q(peers[i].has_ip ? peers[i].ip.toString() : "");
    body += "}";
  }
  body += "]";
  body += "}";
...
String healthJson() {
  ...
  body += ",\"tdm_slot\":" + String(config.tdm_slot);
  body += ",\"tdm_total\":" + String(config.tdm_total);
  body += ",\"peer_count\":" + String(peer_count);
...
void connectWifi() {
  if (config.ssid.length() == 0) {
    Serial.println("[wifi] ssid is empty; configure over Serial first");
    return;
  }
  ...
  Serial.printf("[wifi] connecting to %s as node=%u host=%s\n", config.ssid.c_str(), config.node_id, hostname().c_str());
...
void IRAM_ATTR onCsi(void *, wifi_csi_info_t *info) {
  ...
  sample.tx_node_id = peerIdByMac(info->mac);
  sample.flags = kFlagCsiPresent;
  if (isBroadcastMac(info->mac)) {
    sample.flags |= kFlagBroadcast;
  }
...
void setupEspNow() {
  ...
  for (size_t i = 0; i < peer_count; ++i) {
    if (peers[i].node_id == config.node_id) {
      continue;
    }
    addEspNowPeer(peers[i].mac);
  }
  esp_now_ready = true;
  Serial.printf("[espnow] ready peers=%u\n", static_cast<unsigned>(peer_count));
}
...
void sendHealthPacket() {
  ...
  packet[18] = static_cast<uint8_t>(peer_count);
...
void sendSoundingToPeer(const PeerConfig &peer) {
  if (!esp_now_ready || peer.node_id == 0 || peer.node_id == config.node_id) {
    return;
  }
  SoundingPayload payload = {
      kSoundingMagic,
  ...
  for (uint8_t i = 0; i < kBurstPerPeer; ++i) {
    const esp_err_t err = esp_now_send(peer.mac, reinterpret_cast<const uint8_t *>(&payload), sizeof(payload));
    if (err != ESP_OK) {
...
void runTdmSounding() {
  ...
  for (size_t i = 0; i < peer_count; ++i) {
    sendSoundingToPeer(peers[i]);
  }
}

void handleRoot() {
  server.send(200, "text/plain", "CCTV RF CSI node is running. Use /health, /config, /scan, /sound.\n");
}
...
void handleSound() {
  ...
  server.send(200, "application/json", "{\"status\":\"sound_sent\"}");
}
...
void setupHttp() {
  ...
  // RuView-compatible stimulator endpoint used by the backend.
  compat_server.on("/ota/status", HTTP_GET, handleCompatStatus);
  compat_server.on("/sound", HTTP_GET, []() {
    for (size_t i = 0; i < peer_count; ++i) {
      sendSoundingToPeer(peers[i]);
    }
    compat_server.send(200, "application/json", "{\"status\":\"sound_sent\"}");
  });
  compat_server.begin();
...
void logStatus() {
  ...
  espnow_tx_fail = esp_now_tx_fail_count;
  portEXIT_CRITICAL(&espnow_mux);
  Serial.printf("[status] node=%u ip=%s rssi=%d ch=%d peers=%u csi=%d espnow=%d espnow_rx=%lu tx_ok=%lu tx_fail=%lu tx_submit_fail=%lu dropped=%lu heap=%u\n",
                config.node_id,
                ipAddress().c_str(),
...
void setup() {
  ...
  Serial.printf("[boot] cctv-rf-csi-node version=0.2.0 node=%u label=%s mac=%s\n",
                config.node_id,
                config.label.c_str(),
                macAddress().c_str());
  Serial.printf("[boot] tdm=%u/%u target=%s:%u peers=%u flash=%u psram=%u\n",
                config.tdm_slot,
                config.tdm_total,
  ...
  setupHttp();
  Serial.println("[serial] commands: SHOW, SET key value, REBOOT, FACTORY_RESET");
}
